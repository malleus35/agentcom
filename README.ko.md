...
- JSON 출력에는 `path`, `status`, `agents_md`가 포함될 수 있습니다.
...
지원하는 코딩 에이전트 CLI용 `SKILL.md` 파일을 생성합니다.
...
- `--agent` - 대상 에이전트: `claude|codex|gemini|opencode|all` (기본값 `all`)
